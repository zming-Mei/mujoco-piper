...
class IKSolver:
    ...
    def solve(self, qpos, target_pose: pin.SE3):
        """
        Solve IK problem
        - qpos: current joint angles (length 7 or 8)
        - target_pose: pinocchio.SE3 type
        return 8-dimensional joint_ctrl
        """
        ...
        if qpos.size == nq:
            ...
            raise ValueError(f"qpos dimension should be {nq} or {nq - 1}, but got {qpos.size}")
...
class IKOptimizer:
    def __init__(self, urdf_path: str, ee_frame_name: str = 'grasp_link'):
        ...
        self.target_translation = np.array([0.155, 0.0, 0.222])  # ✅ Updated initial end-effector position
        ...
        self.q_current = np.zeros(8)  # Initial joint angles
...
class PiperGraspEnv:
    ...
    def get_joint(self):
        """
        Get current joint states of the robot arm
        """
    ...
    def gripper_control(self, *args):
        if len(args) == 1 and isinstance(args[0], float):
            self.q_current[-1] = args[0]
            self.step(self.q_current)
        elif len(args) == 1 and isinstance(args[0], bool):
            if args[0] is True:  # open
                for i in range(1000):
    ...
    def run_circle_trajectory(self, center, radius, angular_speed, duration):
        """
        Move the robot end-effector along a circular trajectory

        Args:
            center: center position of the circle (np.array [x, y, z])
        ...
        while time.time() - start_time < duration:
            ...
            # Compute circular trajectory in XY plane
            x = center[0] + radius * np.cos(angle)
            y = center[1] + radius * np.sin(angle)
            z = center[2]
            # Fixed orientation (or you can rotate synchronously)
            euler = R.from_matrix(self.target_rotation).as_euler('xyz', degrees=False)
            rx, ry, rz = euler[0], euler[1], euler[2]
            # Call move_to_position to control the robot
